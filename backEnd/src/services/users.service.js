...
const getUser = async (_id) => {
  try {
    const user = await User.aggregate([
      { $match: { $and: [{ _id: ObjectId(_id) }, { isDeleted: 0 }] } },
      {
        $lookup: {
          from: "customers",
          localField: "_id",
    ...
    return !!user.length ? user[0] : new UserError(404, "User Not Found");
  } catch (e) {
    return new UserError();
  }
};
...
const createUser = async (body, reqUser) => {
  try {
    ...
    if (exists) return new UserError(401, "User already exists");
...
const deleteUser = async (_id) => {
  try {
    ...
    await session.withTransaction(async () => {
      // del customer
      await Customer.findOneAndDelete({ userId: _id });
      // del user
      await User.findByIdAndDelete({ _id: _id });
    });

    session.endSession();
    return { message: `Delete successfully with id:${_id}` };
  } catch (e) {
    return new UserError();
  }
};
